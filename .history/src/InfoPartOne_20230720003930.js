...
export class InfoPartOne extends Component {
    ...
    onChangeDataPassport(e) {
        let date14ago = new Date();
        date14ago.setFullYear(date14ago.getFullYear() - 14);

        let userDate = new Date(e);

        if(userDate > date14ago) {
            this.setState({textDataPassport: 'нельзя подписать договор, если вам меньше 18 лет'})
        }
        else {
    ...
    render() {
        return (
            <div>
                ...
                // value={this.state.inputPassport}
                 />
